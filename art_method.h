...
class ArtMethod FINAL {
 public:
  ...
  void SetEntryPointFromJni(const void* entrypoint) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    mirror::Class* java_class = GetDeclaringClass();
    LOG(WARNING) << "SetEntryPointFromJni: Class:\"" << PrettyClass(java_class) << "\" Method:\"" << PrettyMethod(this)
    << "\" Entry:\"" << std::addressof(entrypoint) << "\"" ;
